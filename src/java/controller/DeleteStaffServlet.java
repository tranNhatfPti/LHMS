...
public class DeleteStaffServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (staffDAO.deleteStaffByID(staffID)) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (staffIds != null && staffIds.length > 0) {
            for (String i : staffIds) {
                staffDAO.deleteStaffByID(Integer.parseInt(i));
            }
            response.sendRedirect("list-staff");
        } else {
            request.setAttribute("notice", "Xóa nhân viên không thành công !");
            request.getRequestDispatcher("view/manager/list-staff.jsp").forward(request, response);
        }
