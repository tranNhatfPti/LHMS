    ...
    <body onload="time()" class="app sidebar-mini rtl">
        ...
        <aside class="app-sidebar">
            ...
            <ul class="app-menu">
                ...
                <li><a class="app-menu__item" href="table-data-money.html"><i class='app-menu__icon bx bx-dollar'></i><span class="app-menu__label">Phát sinh</span></a></li>
                ...
                <li><a class="app-menu__item" href="page-calendar.html"><i class='app-menu__icon bx bx-calendar-check'></i><span class="app-menu__label">Tính tiền </span></a></li>
        ...
        <main class="app-content">
            <h2>Thay đổi thông tin trọ </h2>
            <div>
                <c:set var="p" value="${requestScope.lodgingHouse}" />
                <form id="addLodgingForm" action="update-info-lodging-house?lodgingHouseId=${p.getLodgingHouseId()}" method="post">
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="tenNhaTro">Tên nhà trọ<span style="color:red;">(*)</span></label>
                            <input name="nameLodging" type="text" class="form-control" id="tenNhaTro" placeholder="Nhập tên nhà trọ" value="${p.getNameLodgingHouse()}" required>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="tinh" name="tinh" title="Chọn Tỉnh Thành" required>
                                <option value="0">${p.getProvince()}</option>
                                <!-- Populate with options dynamically if applicable -->
                            </select>
                            <input type="hidden" name="currentProvince" id="currentProvince" value="${p.getProvince()}">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="quan" name="quan" title="Chọn Quận Huyện" required>
                                <option value="0">${p.getDistrict()}</option>
                                <!-- Populate with options dynamically if applicable -->
                            </select>
                            <input type="hidden" name="currentDistrict" id="currentDistrict" value="${p.getDistrict()}">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="phuong" name="phuong" title="Chọn Phường Xã" required>
                                <option value="${requestScope.wardID}">${p.getWard()}</option>
                                <!-- Populate with options dynamically if applicable -->
                            </select>
                            <input type="hidden" name="currentWard" id="currentWard" value="${requestScope.wardID}">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="addressDetail" class="form-control" id="addressDetail" placeholder="Nhập địa chỉ cụ thể"  value="${p.getAddressDetail()}"  required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="numberOfRoom">Số lượng phòng<span style="color:red;"></span></label>
                            <input type="number" name="numberOfRoom" class="form-control" id="numberOfRoom" placeholder="Nhập số phòng" min="1" step="1" value="${p.getNumberOfRoom()}" required>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="status" name="status" title="Chọn Trạng thái" required>
                                <option value="${p.isStatus()}">
                                    <c:choose>
                                        <c:when test="${p.isStatus()}">Còn phòng</c:when>
                                        <c:otherwise>Hết phòng</c:otherwise>
                                    </c:choose>
                                </option>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input name="avatar" type="text" id="avatar" style="display: none" value="${p.getImg()}">
                            <div id="imgDiv"></div>
                        </div>
                    </div>
                    <button type="submit" class="btn btn-primary">Thay đổi thông tin trọ</button>
                </form>
        ...
        <script>
            document.getElementById('addLodgingForm').addEventListener('submit', function (event) {
                ...
                function checkValidName(errorMessageid) {
                    ...
                    const input = document.getElementById("tenNhaTro").value;
                    if (input === '') {
                        // Nếu ô input trống, hiển thị thông báo lỗi
                        document.getElementById('form-message').textContent = 'Vui lòng nhập tên trọ .';
                        return false;
                    } else if (input.split(' ').some(word => word === '')) {
                        // Kiểm tra nếu có từ nào là khoảng trắng
                        document.getElementById('form-message').textContent = 'Tên trọ không thể chứa khoảng trắng ở đầu hoặc cuối.';
                        return false;
                    } else {
...
// save this link somewhere, e.g. put it in an input field
        ...
        <script>
            ...
            document.addEventListener("DOMContentLoaded", function () {
                const numberOfRoomInput = document.getElementById("numberOfRoom");

                // Add an event listener to validate the input on form submission
                document.getElementById("addLodgingForm").addEventListener("submit", function (event) {
                    const numberOfRooms = numberOfRoomInput.value;

                    // Check if the value is a positive integer
                    if (!/^[1-9]\d*$/.test(numberOfRooms)) {
                        event.preventDefault(); // Prevent form submission
                        alert("Vui lòng nhập một số nguyên dương cho số lượng phòng."); // Show error message
                        numberOfRoomInput.focus(); // Focus the input field
                    }
                });
            });
            $(document).ready(function () {
                var currentProvince = "${p.getProvince()}";
                var currentDistrict = "${p.getDistrict()}";
                var currentWard = "${p.getWard()}";

                // Fetch provinces
                $.getJSON('https://esgoo.net/api-tinhthanh/1/0.htm', function (data_tinh) {
                    if (data_tinh.error == 0) {
                        $.each(data_tinh.data, function (key_tinh, val_tinh) {
                            if (val_tinh.full_name !== currentProvince) {
                            ...
                            $.getJSON('https://esgoo.net/api-tinhthanh/2/' + idtinh + '.htm', function (data_quan) {
                                if (data_quan.error == 0) {
                                    ...
                                    $.each(data_quan.data, function (key_quan, val_quan) {
                                        if (val_quan.full_name !== currentDistrict) {
                                        ...
                                        $.getJSON('https://esgoo.net/api-tinhthanh/3/' + idquan + '.htm', function (data_phuong) {
                                            if (data_phuong.error == 0) {
                                                ...
                                                $.each(data_phuong.data, function (key_phuong, val_phuong) {
                                                    if (val_phuong.full_name !== currentWard) {
