                    ...
                    <fieldset >
                        ...
                        <div class="form__radios" style="margin-top: 5px">
                            <div class="form__radio" style="box-shadow: 0 0 1em #8f8080; display: inline-flex">
                                ...
                                <%
                                    Date date = formatter.parse(billNeedPay.getMonthYear());
                                    Contract contract = cd.getContractByDateAndRoomID(billNeedPay.getRoomId(), date);
                                    int id = contract.getTenantId().getAccountID();
                                    Account accountIsRenting = ad.getAccountById(id);
                                    InformationOfUser inforAccount = iud.getInformationByAccountID(id);
                                %>
                                <div style="margin-left: 50px">
                                    <div>
                                        <strong><%=inforAccount.getFullName()%></strong>
                                    </div>
                                    <div style="margin: 10px 0">  
                                        <strong><%=accountIsRenting.getEmail()%></strong>
                                    </div>
                                    <div style="margin-bottom: 10px">           
                                        <strong><%=inforAccount.getPhoneNumber()%></strong>
                                    </div>
                                    <div>           
                                        <strong><%=billNeedPay.getRoomId()%></strong>
                                    </div>
                                </div>
                            </div>          
                        </div>
                    ...
                    <fieldset style="margin: 15px 0">
                        ...
                        <div class="form__radios" style="margin-top: 5px; width: 50%">
                            ...
                            <div class="iphone" style="margin-left: 20px">
                                ...
                                <form action="/ManageLodgingHouse/vnpayajax" class="form" id="frmCreateOrder" method="post">        
                                    <div>
                                        ...
                                        <table>
                                            <tbody>
                                                <tr>
                                                    <td>Tiền điện</td>
                                                    <td align="right" class="price"><%=billNeedPay.getPriceElectronic()%></td>
                                                </tr>
                                                <tr>
                                                    <td>Tiền nước</td>
                                                    <td align="right" class="price"><%=billNeedPay.getPriceWater()%></td>
                                                </tr>
                                                <tr>
                                                    <td>Tiền phòng</td>
                                                    <td align="right" class="price"><%=rd.getRoomsById(billNeedPay.getRoomId()).getPrice()%></td>
                                                </tr>
                                                <%
                                                    List<ServiceOfRoom> listSOR = srd.getAllServiceOfRoomByRoomId(billNeedPay.getRoomId());
                                                    for(ServiceOfRoom sr : listSOR){
                                                        if(sr.getServiceId() != 1 && sr.getServiceId() != 2){
                                                %>
                                                <tr>
                                                    <td>Tiền <%=sd.getServiceById(sr.getServiceId()).getServiceName()%></td>
                                                    <td align="right" class="price"><%=sr.getPrice()%></td>
                                                </tr>
                                                <%}}%>    
                                                <tr>
                                                    <td>Tiền đã trả</td>
                                                    <td align="right" class="price" style="color: green"><%=billNeedPay.getPaid()%></td>
                                                </tr>
                                            </tbody>                        
                                        </table>
                                        <hr>
                                        <div style="background-color: #ffdbdb;
                                            ...
                                            <Strong align="right" style="color: crimson">                                       
                                                <%=billNeedPay.getMissing()%>
                                            </Strong>
                                        </div>
                                    </div>
                                    <div style="display: flex; justify-content: center">
                                        <button class="button button--30" type="submit" onclick="printBill()">
                                            </svg>Thanh toán</button>
                                        <input type="hidden" name="bankCode" value="">
                                        <input type="hidden" name="amount" value="<%=billNeedPay.getMissing()%>">
                                        <input type="hidden" name="roomId" value="<%=billNeedPay.getRoomId()%>">
                                        <input type="hidden" name="monthYear" value="<%=billNeedPay.getMonthYear()%>">
                                    </div>
                                </form>
                            </div>
                        </div>
